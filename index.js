...
if (process.versions.fibjs) {
    ...
    async function _fetch(href, opts) {
        ...
        const resp = await opts.agent.request(href, opts);
    ...
    k8s = await sbox.import('@kubernetes/client-node', import.meta.dirname);
    k8s.KubeConfig.prototype.createAgent = function (cluster, agentOptions) {
        return new http.Client(agentOptions);
    };
} else {
    k8s = await import('@kubernetes/client-node');
}

// Object.keys(k8s).forEach(name => console.log(`${name},`));

// 导出所有的 k8s 模块内容作为命名导出
export const {
    ADD,
    AbortError,
